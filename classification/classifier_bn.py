...
for i in range(start_subject_index, len(unique_subject_list)):
    ...
    if model_choice == 'ATCNet':
        ...
        model = ATCNet_(nb_classes=nb_classes, Chans=X_train.shape[2], Samples=X_train.shape[3])
    elif model_choice == 'EEGNet':
         # Ensure EEGNet params match input shape: (trials, chans, samples, 1)
         model = EEGNet(nb_classes=nb_classes, Chans=X_train.shape[1], Samples=X_train.shape[2])
    else:
...
if fully_completed:
    ...
    if model_choice == 'ATCNet':
        final_model = ATCNet_(nb_classes=nb_classes, Chans=X.shape[1], Samples=X.shape[2])
    elif model_choice == 'EEGNet':
        final_model = EEGNet(nb_classes=nb_classes, Chans=X.shape[1], Samples=X.shape[2])
    else:
