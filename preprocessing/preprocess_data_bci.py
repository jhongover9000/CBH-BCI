...
# Data Version
data_version = 'bci_subject_data_v1' # Updated version
...
rest_tmin, rest_tmax = -1.0, 0.0 # E.g., 1 second ending at MI onset
...
baseline_rest = (-1.5, -1.0) # Baseline for Rest: -1.5 seconds to -1.0 seconds relative to MI onset
...
def process_subject_data(set_file_path, subject_id, new_freq, f_low, f_high,
    ...
    mi_data_processed = mi_processed.get_data(copy=True)
...
for idx, data in enumerate(all_mi_data):
    print(f"Subject index {idx}: data shape {data.shape}")


# --- Combine data from all subjects ---
# Use np.vstack for epoch data (epochs, channels, time)
# Use np.concatenate for labels and subject IDs (flat arrays)
if all_mi_data:
    ...
    print("Final MI Data Shape:", X_mi_final.shape)
    print("Final MI Labels Shape:", y_mi_final.shape)
    print("Final MI Subject IDs Shape:", subject_mi_final.shape)

    # Save the processed MI data
    save_filename_mi = f"mit_mi_epoched_derived_rest_{data_version}.npz"
    np.savez(os.path.join(save_dir, save_filename_mi), X=X_mi_final, y=y_mi_final, subject_ids=subject_mi_final)
    print(f"MI epoched data saved to {os.path.join(save_dir, save_filename_mi)}")
else:
    print("No MI data collected from any subject.")
    X_mi_final = np.array([]) # Define as empty for consistency
    y_mi_final = np.array([])
    subject_mi_final = np.array([])
...
if all_rest_data:
    ...
    print("Final Rest Data Shape:", X_rest_final.shape)
    print("Final Rest Labels Shape:", y_rest_final.shape)
    print("Final Rest Subject IDs Shape:", subject_rest_final.shape)

    # Save the processed Rest data
    save_filename_rest = f"mit_rest_epoched_derived_{data_version}.npz"
    np.savez(os.path.join(save_dir, save_filename_rest), X=X_rest_final, y=y_rest_final, subject_ids=subject_rest_final)
    print(f"Rest epoched data (derived) saved to {os.path.join(save_dir, save_filename_rest)}")
else:
    print("No derived Rest data collected from any subject.")
    X_rest_final = np.array([]) # Define as empty for consistency
    y_rest_final = np.array([])
    subject_rest_final = np.array([])
