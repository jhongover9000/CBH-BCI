...
def process_subject_data(set_file_path, subject_id, new_freq, f_low, f_high,
    ...
    # --- Identify MI Epochs that contain the key ---
    matching_event_ids = []
    for event_id in epochs.event_id:
        if mi_event_key in event_id:
            matching_event_ids.append(event_id)
    
    if not matching_event_ids:
        print(f"  Warning: No event markers containing '{mi_event_key}' found in event IDs: {epochs.event_id}. Skipping.")
        return None, None, None, None, None, None
    
    print(f"  Found {len(matching_event_ids)} event markers containing '{mi_event_key}': {matching_event_ids}")
    
    # Create a combined selection of all epochs that match any of the matching event IDs
    try:
        mi_epochs_orig = epochs[matching_event_ids].copy()
        print(f"  Selected {len(mi_epochs_orig)} epochs for markers containing '{mi_event_key}'. Epoch time range: [{mi_epochs_orig.tmin:.3f}, {mi_epochs_orig.tmax:.3f}]s")
        if len(mi_epochs_orig) == 0:
            print(f"  Warning: No epochs found for markers containing '{mi_event_key}'. Skipping.")
            return None, None, None, None, None, None
    except KeyError as e:
        print(f"  Error selecting epochs: {e}")
        return None, None, None, None, None, None
...
for subject_num in range(1, num_subjects + 1):
    ...
    rest_data, rest_labels, rest_subject_ids = process_subject_data(
        ...
        select_chan, chan2use, drop_chan, chan2drop,
        mi_event_key, rest_tmin, rest_tmax, baseline_mi, baseline_rest
    )
