...
def ATCNet_(n_classes, in_chans = 22, in_samples = 1125, n_windows = 5, attention = 'mha', 
    ...
    """ ATCNet model from Altaheri et al 2023.
        Modified to ensure adequate sequence length for TCN processing.
    """
    ...
    def get_last_channel_shape(input_shape):
        return (input_shape[0], input_shape[1], input_shape[3])
        
    block1 = Lambda(lambda x: x[:,:,-1,:], 
                   output_shape=get_last_channel_shape)(block1)
    
    # Get current sequence length after Conv_block
    seq_length = block1.shape[1]
    print(f"Sequence length after Conv_block: {seq_length}")
    
    # Calculate minimum window size needed for TCN based on kernel size and depth
    # For TCN with kernel size K and depth D, receptive field is (K-1)*2^D + 1
    min_window_size = max(tcn_kernelSize, 4)  # Ensure at least 4 time steps
    
    # Adjust n_windows if sequence is too short
    actual_n_windows = min(n_windows, max(1, seq_length - min_window_size + 1))
    if actual_n_windows < n_windows:
        print(f"Warning: Reducing sliding windows from {n_windows} to {actual_n_windows} due to sequence length constraints")
       
    # Sliding window 
    sw_concat = []   # to store concatenated or averaged sliding window outputs
    for i in range(actual_n_windows):
        # Ensure window has minimum size for TCN processing
        st = i
        end = block1.shape[1] - actual_n_windows + i + 1
        
        # Print window sizes for debugging
        print(f"Window {i+1}: start={st}, end={end}, size={end-st}")
        ...
        # Define output shape function for the Lambda layer that takes the last sequence
        def get_output_shape(input_shape):
            return (input_shape[0], input_shape[2])
        
        # Get feature maps of the last sequence with explicit output_shape
        block3 = Lambda(lambda x: x[:,-1,:], output_shape=get_output_shape)(block3)
...
def TCN_block_(input_layer, input_dimension, depth, kernel_size, filters, dropout,
    ...
    """ TCN_block from Bai et al 2018
        ...
        Notes
        -----
        Modified to ensure padding is always 'causal' and proper dilation rates
        to prevent kernel size issues with small input sequences
    """    
    
    # Get sequence length to check if it's too small for the kernel
    seq_length = input_layer.shape[1]
    
    # Adjust kernel size if needed to prevent negative dimension issues
    # If sequence length is small, reduce kernel size
    effective_kernel_size = min(kernel_size, seq_length)
    
    # Make sure we use causal padding to maintain sequence length
    block = Conv1D(filters, kernel_size=effective_kernel_size, dilation_rate=1, activation='linear',
                    kernel_regularizer=L2(weightDecay),
                    kernel_constraint = max_norm(maxNorm, axis=[0,1]),
                    padding='causal',  # Causal padding ensures output length = input length
    ...
    # For deeper layers, be careful with dilation rates when sequence is short
    for i in range(depth-1):
        # Calculate effective dilation rate - don't let it get too large for small sequences
        dilation_rate = min(2**(i+1), max(1, seq_length // effective_kernel_size))
        
        block = Conv1D(filters, kernel_size=effective_kernel_size, dilation_rate=dilation_rate, activation='linear',
                    kernel_regularizer=L2(weightDecay),
                    kernel_constraint = max_norm(maxNorm, axis=[0,1]),
                    padding='causal',  # Causal padding is crucial here
